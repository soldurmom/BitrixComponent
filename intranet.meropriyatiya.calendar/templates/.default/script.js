...
JCCalendarInfoWin.prototype.deleteEntry = function()
{
	if (confirm(jsBXCalendarAdmin.DELETE_CONFIRM + ' "' + BX.util.htmlspecialcharsback(this.INFO.ENTRY.NAME) +'"?'))
	{
		...
		BX.ajax.get(
			'/bitrix/tools/intranet_meropriyatiya.php?action=delete'+ '&sessid=' + BX.message('bitrix_sessid') + '&absenceID='+ this.INFO.ENTRY.ID + '&js=1',
			BX.delegate(
				function()
				{
		...
		BX.AbsenceCalendar.popup = new BX.PopupWindow("BXAbsence", null, {
			...
			events: {
				onAfterPopupShow: function()
				{
					...
					BX.ajax.post(
						'/bitrix/tools/intranet_meropriyatiya.php',
						{
